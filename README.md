...
### 疑似的な辞書作成プログラム  
テキストファイルを与え,品詞を指定すると,頻出する単語を抽出できます.  
また,好きな要素(読み,意味など)を抽出したデータに追加できます.  

### ポイント
文章などを疑似的に要約できます.   
頻出のものだけ抽出するので,重要度を把握できます.  
...
```
またテキストファイル(.txt)を用意する必要があります.  
アップロードが面倒な場合は以下を実行してください.  
```sh
%%bash
cat <<EOF > input3.txt
#テキストを入力
...
#### お断り  
あまり汎用性がないのとインターフェースがとにかく使いずらいです.  
加えてjanomeの動作に苦戦したのでプログラムが荒いです.  
ご容赦ください.  
...
colab環境verでは処理軽減のため一部コードをコメントアウトしています.  
稼動しそうであれば,コメントアウトを外してください.  
